...
def prepCases(settings, ids):
    ...
    if settings['file']['LGI1_cases.ped'].split('/')[-1] not in files:
        print('Preparing cases data')
        count = 0
        for file in files:
            if '.ped' in file:
                print('Current file: ' + file)
                with open(settings['file']['LGI1_cases.ped'], 'a') as outFile:
        ...
        count = subprocess.run(['bash', settings['util']['countRows'],\
             settings['file']['LGI1_cases.ped']], stdout=subprocess.PIPE)
