...
def getrsIDs(settings):
    ...
    with open(settings['file']['LGI1_controls.map'], 'r') as inFile:
    ...
    cases = list()
    with open(settings['file']['LGI1_cases.map'], 'r') as inFile:
...
def main():
    ...
    subprocess.call(['cp', settings['file']['LGI1_cases.map'], '../../LGI1.map'], stdout=subprocess.PIPE)
    print("LGI1.map is LGI1_cases.map")
